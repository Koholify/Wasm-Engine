...
static void _set_item(kc_set set, size_t val, bool isNew) {
	size_t i = _set_get_hash(set, val) % kc_set_cap(set);
	kc_set_item newItem = set.data[i];
	if(!newItem.is_used || (newItem.is_used && newItem.item == val)) {
...
kc_set kc_set_copy(kc_set set) {
	kc_set newItem = kc_set_init(kc_set_cap(set));
	kc_set_iterator it = kc_set_iter(set);
	size_t val;
	while (kc_set_next(&it, &val))
		kc_set_set(&newItem, val);
	return newItem;
}
...
static void _strset_item(kc_strset set, const char* val, bool isNew) {
	size_t i = _strset_get_hash(set, val) % kc_strset_cap(set);
	kc_strset_item newItem = set.data[i];
	if(!newItem.is_used || (newItem.is_used && !strncmp(newItem.item, val, STR_HASH_LENGTH))) {
		if (isNew && !newItem.is_used) {
			kc_arr_len(set.data)++;
		}
...
kc_strset kc_strset_copy(kc_strset set) {
	kc_strset newItem = kc_strset_init(kc_strset_cap(set));
	kc_strset_iterator it = kc_strset_iter(set);
	const char* val;
	while (kc_strset_next(&it, &val))
		kc_strset_set(&newItem, val);
	return newItem;
}
