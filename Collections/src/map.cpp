...
static void _set_item(kc_map map, size_t key, void* value, bool isNew) {
	...
	kc_map_item newItem = map.data[i];
	if(!newItem.is_used || (newItem.is_used && newItem.key == key)) {
		if (isNew && !newItem.is_used) {
			kc_arr_len(map.data)++;
		}

		newItem.is_used = true;
		newItem.key = key;
		newItem.value = value;
		map.data[i] = newItem;
	} else {
...
kc_map kc_map_copy(kc_map map) {
	kc_map newMap = kc_map_init(kc_map_cap(map));
	kc_map_iterator it = kc_map_iter(map);
	kc_map_item* val;
	while (kc_map_next(&it, &val))
		kc_map_set(&newMap, val->key, val->value);
	return newMap;
}
...
static void _strset_item(kc_strmap map, const char* key, void* value, bool isNew) {
	...
	kc_strmap_item newItem = map.data[i];
	if(!newItem.is_used || (newItem.is_used && !strncmp(newItem.key, key, STR_HASH_LENGTH))) {
		if (isNew && !newItem.is_used) {
			kc_arr_len(map.data)++;
		}
...
kc_strmap kc_strmap_copy(kc_strmap map) {
	kc_strmap newMap = kc_strmap_init(kc_strmap_cap(map));
	kc_strmap_iterator it = kc_strmap_iter(map);
	kc_strmap_item* val;
	while (kc_strmap_next(&it, &val))
		kc_strmap_set(&newMap, val->key, val->value);
	return newMap;
}
